...
describe("Identity", () => {
    describe("Create identity", () => {
        it("Should not create a identity if the strategy is wrong", () => {
            const fun = () => new Identity("wrong" as any)
        ...
        it("Should not create a identity with a message strategy if there is no message", () => {
            const fun = () => new Identity(Strategy.MESSAGE)

            expect(fun).toThrow("The message is not defined")
        })

        it("Should not create a identity with a message strategy if the message is not a string", () => {
            const fun = () => new Identity(Strategy.MESSAGE, 1 as any)

            expect(fun).toThrow("The message is not a string")
        })

        it("Should create a identity with a message strategy", () => {
        ...
        it("Should not create a identity with a serialized strategy if there is no serialize identity", () => {
            const fun = () => new Identity(Strategy.SERIALIZED)

            expect(fun).toThrow("serialized identity is not defined")
        })

        it("Should not create a identity with a serialized strategy if the serialize identity cannot be parsed", () => {
            const fun = () => new Identity(Strategy.SERIALIZED, "{a}")

            expect(fun).toThrow("The serialized identity cannot be parsed")
        })

        it("Should not create a identity with a serialized strategy if the serialize identity does not contain the right parameters", () => {
            const fun = () =>
                new Identity(Strategy.SERIALIZED, {
        ...
        it("Should create a identity with a serialized strategy", () => {
            const identity1 = new Identity()
            const serialized = identity1.serializeIdentity()
            const identity2 = new Identity(Strategy.SERIALIZED, serialized)
